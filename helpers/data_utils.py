...
def concat_all_csv_files(data_path, group_size, model_type=None):
    all_csv_file_paths = [os.path.join(data_path, f)
                          for f in os.listdir(data_path)
                          if f.endswith('.csv') and model_type in f] if model_type is not None \
        else [os.path.join(data_path, f)
              for f in os.listdir(data_path)
              if f.endswith('.csv')]
    all_data_list = []
